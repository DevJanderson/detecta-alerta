...
import type { AuthUser } from '../../../layers/auth/app/composables/types'
...
describe('UserModel', () => {
  describe('createUserModel()', () => {
    ...
    it('extrai nomes de grupo', () => {
      const model = createUserModel(makeUser())
      expect(model.groups).toEqual(['administradores', 'editores'])
    })
  ...
  describe('funções de permissão', () => {
    ...
    it('userHasGroup verifica nome', () => {
      expect(userHasGroup(model, 'administradores')).toBe(true)
