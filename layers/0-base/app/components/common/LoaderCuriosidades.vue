...
<template>
  <div class="flex flex-col items-center gap-10">
    ...
    <Transition name="curiosidade" mode="out-in">
      ...
      >
        <div class="flex items-center gap-2 text-primary-950">
          <Icon name="lucide:lightbulb" class="size-6" />
          <p class="text-base font-semibold leading-tight">Voce sabia?</p>
        </div>

        <p class="text-xl leading-relaxed text-primary-950">
          {{ current.texto }}
        </p>

        <p v-if="current.fonte" class="text-xs text-primary-950">
    ...
    <div class="relative size-50">
      ...
      <div class="absolute inset-0 flex items-center justify-center">
        <div class="-rotate-45">
          <div class="size-50 rounded-[36px] bg-primary-100 losango-breathe" />
        </div>
      </div>
      ...
      <div class="absolute inset-0 flex items-center justify-center">
        <div class="-rotate-45">
          <div class="size-35 rounded-[36px] bg-primary-900 losango-pulse" />
        </div>
      </div>
    </div>

    <!-- Texto de progresso -->
    <p class="flex items-baseline gap-1 text-xl font-semibold text-secondary-900">
