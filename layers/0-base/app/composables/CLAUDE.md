...
| --------------- | -------------------------------- | ----------------------------------------- |
| **Utils**       | `layers/0-base/app/utils/`       | Funções puras, sem estado, sem Vue        |
| **Composables** | `layers/0-base/app/composables/` | Lógica com estado reativo (ref, computed) |
...
// ✅ CORRETO - Util em layers/0-base/app/utils/
...
```typescript
// layers/0-base/app/composables/useLoading.ts
...
```typescript
// layers/0-base/app/composables/usePagination.ts
...
```typescript
// layers/0-base/app/composables/useDebounce.ts
...
```typescript
// layers/0-base/app/composables/useToggle.ts
...
```typescript
// layers/0-base/app/composables/useField.ts
...
```typescript
// layers/0-base/app/composables/useGlobalLoading.ts
...
```typescript
// layers/0-base/app/composables/useWindowSize.ts
...
```typescript
// layers/0-base/app/composables/useAsyncState.ts
interface UseAsyncStateOptions<T> {
  immediate?: boolean
  initialValue?: T
}

export function useAsyncState<T>(fn: () => Promise<T>, options: UseAsyncStateOptions<T> = {}) {
...
const {
  ...
  isLoading,
  execute: refresh
} = useAsyncState(() => $fetch<User[]>('/api/users'))
```
...
import { useToggle } from '~/layers/0-base/app/composables/useToggle'
...
import { useDebounce } from '~/layers/0-base/app/composables/useDebounce'
...
import { describe, it, expect } from 'vitest'
import { useLoading } from '~/layers/0-base/app/composables/useLoading'
