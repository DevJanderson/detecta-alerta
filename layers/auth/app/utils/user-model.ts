...
import type { AuthUser, AuthPermissao, AuthGrupo } from '../composables/types'
...
export function createUserModel(user: AuthUser): UserModel {
  const permissions = user.permissoes?.map((p: AuthPermissao) => p.codigo) ?? []
  ...
  return Object.freeze({
    ...
    permissions,
    groups,
    isAdmin: groups.includes('administradores')
  })
}
